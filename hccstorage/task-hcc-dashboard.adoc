...
summary: Você pode visualizar recursos de storage no Dashboard de controle de nuvem híbrida da NetApp. Além disso, você pode monitorar a capacidade de storage e o desempenho do storage. 
keywords: netapp, element, hcc, hybrid cloud control, storage, storage performance, capacity 
---
= Monitore os recursos de storage no dashboard de controle de nuvem híbrida
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Com o dashboard de controle de nuvem híbrida da NetApp, você pode visualizar rapidamente todos os recursos de storage. Além disso, você pode monitorar a capacidade de storage e o desempenho do storage.
...
NOTE: Você pode definir o limite para Aviso, que é padrão para 3% abaixo do limiar de erro. Os limiares de erro e crítico são predefinidos e não podem ser configurados por design. O limiar de erro indica que menos de um nó de capacidade permanece no cluster. Para obter instruções sobre como definir o limite, link:../storage/task_system_manage_cluster_set_the_cluster_full_threshold.html["Definir o limite máximo do cluster"]consulte .

+

TIP: Para obter detalhes sobre a API de elemento de limites de cluster relacionada, consulte link:../api/reference_element_api_getclusterfullthreshold.html[""GetClusterFullThreshold""] a documentação da API de software _Element_. Para exibir detalhes sobre a capacidade de bloco e metadados, consulte link:../storage/concept_monitor_understand_cluster_fullness_levels.html["Compreender os níveis de plenitude do cluster"] a documentação do software _Element_.
...
.. Se a capacidade de armazenamento efetiva se aproximar de um limite de erro ou crítico, considere limpar os dados do sistema.
...
Para obter detalhes sobre o método da API do elemento associado, consulte o link:../api/reference_element_api_getclusterstats.html["GetClusterStats"] método na documentação da API do software _Element_.
