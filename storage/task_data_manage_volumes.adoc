...
NOTE: Os sistemas operacionais diferem na forma como tratam volumes clonados. O VMware ESXi tratará um volume clonado como uma cópia de volume ou um volume de snapshot. O volume estará disponível para ser usado na criação de um novo datastore. Para obter mais informações sobre como montar volumes clonados e lidar com LUNs de snapshot, consulte a link:https://techdocs.broadcom.com/["Documentação da VMware"^] sobre como montar uma cópia de datastore VMFS e gerenciar datastores VMFS duplicados.
