...
Existem tipos de acesso disponíveis através da API que não estão disponíveis na IU do Element.
...
+
Verlink:../api/concept_element_api_about_the_api.html["Referência da API do software NetApp Element"] .
...
link:concept_system_manage_manage_ldap.html["Ative a autenticação LDAP com a interface do usuário Element"]
...
* link:../api/reference_element_api_app_b_access_control.html["Saiba mais sobre os tipos de acesso disponíveis para APIs do Element"]
* link:concept_system_manage_manage_ldap.html["Ative a autenticação LDAP com a interface do usuário Element"]
