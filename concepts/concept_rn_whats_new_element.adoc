...
[role="lead"]
A NetApp atualiza periodicamente os softwares SolidFire e Element para oferecer novos recursos, melhorias e correções de bugs. O Element 12.9 é a versão mais recente e inclui atualizações de segurança e de componentes do sistema, melhorias operacionais e resolução de problemas.


NOTE: As atualizações cumulativas de software e firmware são instaladas como parte de uma atualização do Element 12.9, com base na versão atual do Element em execução em um cluster de armazenamento. Por exemplo, se um cluster estiver executando o Element 12.5, você poderá atualizar diretamente para o Element 12.9 para obter as atualizações cumulativas das versões 12.7, 12.8 e 12.9. Para obter informações sobre os caminhos de atualização suportados, consulte o artigo da Base de Conhecimento. https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/Element_Software/What_is_the_upgrade_matrix_for_storage_clusters_running_NetApp_Element_software["Qual é a matriz de atualização para clusters de storage que executam o software NetApp Element"^] .
...
Saiba mais sobre as novidades do Element 12.9.



[[support-for-fqdn-in-setremotelogginghosts]]
=== Suporte para FQDN em "SetRemoteLoggingHosts"

A partir do Elemento 12.9, você pode usar o `SetRemoteLoggingHosts` Método para configurar um Nome de Domínio Totalmente Qualificado (FQDN) ou um endereço IP como o valor do host de registro para um cluster de armazenamento.  Você pode visualizar o valor do host FQDN na resposta para o `GetRemoteLoggingHosts` método.  Para o Element 12.8 e versões anteriores, você só pode configurar um endereço IP como valor do host de registro.

https://docs.netapp.com/us-en/element-software/api/reference_element_api_loggingserver.html["Saiba mais sobre a configuração do host de registro."]
