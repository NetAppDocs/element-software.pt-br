---
permalink: api/reference_element_api_setnodesslcertificate.html 
sidebar: sidebar 
keywords: ssl,certificate,set,setting,private,key,setnodesslcertificate 
summary: Você pode usar o método SetNodeSSLCertificate para definir um certificado SSL de usuário e uma chave privada para o nó de gerenciamento. 
---
= SetNodeSSLCertificate
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Você pode usar o `SetNodeSSLCertificate` método para definir um certificado SSL de usuário e uma chave privada para o nó de gerenciamento.


NOTE: Depois de usar a API, você deve reinicializar o nó de gerenciamento.



== Parâmetros


NOTE: Você deve chamar esse método para o nó de gerenciamento. Por exemplo:

[listing]
----
https://<management node IP>:442/json-rpc/10.0
----
Este método tem os seguintes parâmetros de entrada:

|===
| Nome | Descrição | Tipo | Valor padrão | Obrigatório 


 a| 
certificado
 a| 
A versão de texto codificada pelo PEM do certificado.*Nota:* ao definir um certificado de nó ou cluster, o certificado deve incluir a extensão extendedKeyUsage para serverAuth. Esta extensão permite que o certificado seja usado sem erros em sistemas operacionais e navegadores comuns. Se a extensão não estiver presente, a API rejeitará o certificado como inválido.
 a| 
cadeia de carateres
 a| 
Nenhum
 a| 
Sim



 a| 
PrivateKey
 a| 
A versão de texto codificada pelo PEM da chave privada.
 a| 
cadeia de carateres
 a| 
Nenhum
 a| 
Sim

|===


== Valores de retorno

Este método não tem valores de retorno.



== Exemplo de solicitação

As solicitações para este método são semelhantes ao seguinte exemplo:

[listing]
----
{
    "method" : "SetNodeSSLCertificate",
    "params" : {
        "privateKey": "-----BEGIN RSA PRIVATE KEY-----\\nInDXvTgXMdMoet4BKnftelrXFKHgGqXJoczq4JWzGSIHNgvkrH60\n-----END RSA PRIVATE KEY-----\n",
        "certificate": "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n"
    },
    "id" : 2
}
----


== Exemplo de resposta

Este método retorna uma resposta semelhante ao seguinte exemplo:

[listing]
----
{
    "id" : 2,
    "result" : {}
}
----